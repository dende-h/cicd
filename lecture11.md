...
|04| `it { should be_enabled }` | 対象のサービスが自動起動設定されていることを確認。 |[使用例へジャンプ](#4gemとして指定のバージョンがインストール確認)|
|05| `it { should be_listening }` | 対象のポートがリッスン状態であることを確認。 |[使用例へジャンプ](#5指定のサービスの状態確認)|
|06| `it { should be_reachable }` | 対象のホストやポートが到達可能であることを確認。 |[使用例へジャンプ](#6指定のポートの状態確認)|
